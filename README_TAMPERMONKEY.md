...
- **突破 20 条结果限制** - 可设置显示更多驱动记录（最大 500 条）
...
| 参数 | 说明 | 默认值 | 示例 |
|------|------|--------|------|
| **显示驱动数量** | 搜索结果返回的最大驱动数量 | 100 | 100、200、500 |
| **驱动类型** | 选择 Game Ready 或 Studio 驱动 | Game Ready | Game Ready / Studio |
...
NVIDIA 默认只返回 20 条结果，此参数可突破该限制。建议设置 100-200 条，设置过大可能影响加载速度。
...
```
[NVDriverHelper] 用户脚本已加载 v2.2
[NVDriverHelper] 拦截器已注入 v2.2
