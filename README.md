...
| **驱动类型** | 选择 Game Ready 或 Studio 驱动 | 全部 | 全部 / Game Ready / Studio |
| **强制 Standard 驱动** | 强制使用 Standard 驱动而非 DCH | 关闭 | 勾选启用 |
| **版本号 (精确匹配)** | 搜索特定版本的驱动 | 空 | 566.36 |
| **版本前缀 (取前2位)** | 匹配版本前缀，只取前2位 | 空 | 57、58 |
...
用于匹配版本前缀，**只取输入内容的前2位**进行匹配，例如：
- 输入 `571` → 取前2位 `57` → 匹配 57 开头的所有版本（570.xx、571.xx、572.xx 等）
- 输入 `58` → 匹配 58 开头的所有版本（580.xx、581.xx 等）
- 输入 `5` → 取前2位 `5` → 可能无法正确匹配，建议输入至少2位
...
```
[NVDriverHelper] 用户脚本已加载 v3.3
[NVDriverHelper] 拦截器已注入 v3.3
