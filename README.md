...
| 参数 | 说明 | 默认值 | 示例 |
|------|------|--------|------|
| **显示驱动数量** | 搜索结果返回的最大驱动数量 | 50 | 20、30、50 |
...
NVIDIA 默认只返回 20 条结果，此参数可突破该限制。**注意：NVIDIA 服务器端有硬性限制，超过 50 条可能导致加载失败，建议设置 50 以内。**
...
```
[NVDriverHelper] 用户脚本已加载 v2.4
[NVDriverHelper] 拦截器已注入 v2.4
