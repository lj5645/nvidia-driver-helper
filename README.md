...
用于匹配版本前缀，搜索某个系列的所有驱动，例如：
- 输入 `580` → 匹配 58 开头的所有版本（580.48、581.19 等）
- 输入 `571` → 匹配 571 开头的所有版本（571.86、571.96 等）
- 输入 `57` → 匹配 57 开头的所有版本（570.xx、571.xx 等）
...
```
[NVDriverHelper] 用户脚本已加载 v3.2
[NVDriverHelper] 拦截器已注入 v3.2
