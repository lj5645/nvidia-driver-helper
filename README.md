...
- **GeForce 驱动类型切换** - 支持 Game Ready 和 Studio 驱动
- **RTX/Quadro 驱动类型切换** - 支持 Production Branch 和 New Feature Branch
...
| **GeForce 驱动类型** | 选择 Game Ready 或 Studio 驱动 | 全部 | 全部 / Game Ready / Studio |
| **RTX/Quadro 分支类型** | 选择专业显卡驱动分支 | 全部 | 全部 / Production Branch / New Feature Branch |
| **强制 Standard 驱动** | 强制使用 Standard 驱动而非 DCH | 关闭 | 勾选启用 |
| **版本号 (精确匹配)** | 搜索特定版本的驱动 | 空 | 566.36 |
| **版本系列** | 搜索某个系列的所有驱动 | 空 | 570 |
...
- **全部** - 显示所有分支的驱动
- **Production Branch** - 稳定版分支，适合大多数用户，提供最佳稳定性和性能
- **New Feature Branch** - 新功能分支，提供最新功能和驱动增强
...
```
[NVDriverHelper] 用户脚本已加载 v3.0
[NVDriverHelper] 拦截器已注入 v3.0
